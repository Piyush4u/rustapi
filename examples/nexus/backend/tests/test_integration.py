...
async def test_full_user_flow():
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        # 3. Create Post
        res = await client.post("/api/posts", json={"content": "Hello World from Pytest!"}, headers=headers)
        assert res.status_code == 201
        post_id = res.json()["id"]
        
        # 4. Create Comment
        res = await client.post(f"/api/posts/{post_id}/comments", json={"content": "Nice post!"}, headers=headers)
        assert res.status_code == 201
