...
from rustapi import JsonResponse, Request, RustApi
...
async def register(request: Request):
    ...
    async with AsyncSessionLocal() as session:
        ...
        return JsonResponse(
            UserResponse.model_validate(new_user).model_dump(mode="json"), status_code=201
        )
...
async def list_posts(request: Request):
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Post)
            .options(
                selectinload(Post.author), selectinload(Post.comments).selectinload(Comment.author)
            )
...
async def create_post(request: Request, user: User = None):
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            return JsonResponse(
                PostResponse.model_validate(new_post).model_dump(mode="json"), status_code=201
            )
    except Exception as e:
        import traceback
...
async def create_comment(request: Request, user: User = None):
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            new_comment = Comment(
                content=comment_data.content, author_id=user.id, post_id=int(post_id)
            )
            ...
            return JsonResponse(
                CommentResponse.model_validate(new_comment).model_dump(mode="json"), status_code=201
            )
    except Exception as e:
        import traceback
